...
public class CRDTOrderTree {
  ...
  private final Map<UUID, CRDTOrderTreeNode> nodes;
...
//    nodes.get(nodeId).update(value);
  ...
  public ArrayList<String> getOrderedElements() {
    ...
    visitPath.push(root);
    lastVisitChildIterators.push(root.getLeftNodes().iterator());
    ...
    while (!visitPath.isEmpty()) {
      ...
      if (iterator.hasNext()) {
        ...
        lastVisitChildIterators.push(nextNode.getLeftNodes().iterator());
        childsToVisit.push(CRDTOrderTreeNode.Side.LEFT);
      } else if (side == CRDTOrderTreeNode.Side.LEFT) {
        lastVisitChildIterators.pop();
        lastVisitChildIterators.push(currentNode.getRightNodes().iterator());
        childsToVisit.pop();
