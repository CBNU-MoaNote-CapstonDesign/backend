...
public class CRDTOrderTreeNode {
  ...
  final private UUID nodeId;

  private String value;

  final private LinkedList<CRDTOrderTreeNode> leftNodes = new LinkedList<>();

  final private LinkedList<CRDTOrderTreeNode> rightNodes = new LinkedList<>();
  ...
  CRDTOrderTreeNode(final UUID nodeId, final String value) {
    this.nodeId = nodeId;
    this.value = value;
  }

  public void addNode(Side side, CRDTOrderTreeNode node) {
    LinkedList<CRDTOrderTreeNode> nodes = side == Side.LEFT ? leftNodes : rightNodes;
    synchronized (nodes) {
      int insertIndex = 0;
      while (insertIndex < nodes.size()
          && UuidComparator.opaqueCompare(nodes.get(insertIndex).getNodeId(), node.getNodeId())
          < 0) {
        insertIndex++;
      }
